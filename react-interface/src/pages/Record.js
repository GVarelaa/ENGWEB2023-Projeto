  ...
  const handleFavorite = async (event, id) => {
    try {
      if (favorites.includes(id)) {
        ...
        toast.success(
          "O acórdão foi removido da lista de favoritos com sucesso!",
          {
            position: toast.POSITION.TOP_CENTER,
          }
        );
        ...
        toast.success(
          "O acórdão foi adicionado à lista de favoritos com sucesso!",
          {
            position: toast.POSITION.TOP_CENTER,
          }
        );
      ...
      toast.error(
        "Não foi possível adicionar o acórdão à lista de favoritos!",
        {
          position: toast.POSITION.TOP_CENTER,
        }
      );
            ...
            <div className="d-flex justify-content-end mb-4">
              {favorites.includes(params.id) ? (
                <Button
                  ...
                  startIcon={<HeartFill />}
                  onClick={(event) => handleFavorite(event, params.id)}
                >
              ...
              {decodedToken.level === "admin" && (
                <>
                  <Button
                    variant="outline-dark"
                    startIcon={<Trash3 />}
                    onClick={(event) => handleShowModal(event, params.id)}
                  >
                    Remover Acórdão
                  </Button>
