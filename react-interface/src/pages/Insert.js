...
function Insert() {
    ...
    const handleAreaTematicaChange = (e, index, area) => {
        if (area === 1) {
            const updatedArea = [...area_tematica]
            updatedArea[index] = e.target.value
            setAreaTematica(updatedArea)
        }
        else if (area === 2) {
            const updatedArea2 = [...area_tematica2]
            updatedArea2[index] = e.target.value
            setAreaTematica2(updatedArea2)
        }
    }
    ...
    const handleAddAreaTematica = (area) => {
        if (area === 1) {
            const updatedArea = [...area_tematica]
            updatedArea.push("")
            setAreaTematica(updatedArea)
    ...
    const handleRemoveAreaTematica = (index, area) => {
        if (area === 1) {
            const updatedArea = [...area_tematica]
                                        ...
                                        {area_tematica.map((area, index) => (
                                            <div key={index}>
                                                <Col className="mb-2 t d-flex align-items-center">
                                                    <Form.Select value={area} onChange={(e) => handleAreaTematicaChange(e, index, 1)}>
                                                        {listaAreaTematica1.map((area, i) => (
                                                            <option key={i} value={area}>{area}</option>
                                                        ))}
                                                    </Form.Select>
                                                    <Link><Trash3 size={20} color='black' className='mx-3' onClick={() => handleRemoveAreaTematica(index, 1)} /></Link>
                                                </Col>
                                            </div>
                                        ))}
                                    </Form.Group>
                                ...
                                {/*
                                    <Form.Group className="mb-3">
                                        ...
                                        {area_tematica2.map((area, index) => (
                                            <div key={index}>
                                                <Col className="mb-2 t d-flex align-items-center">
                                                    <Form.Select value={area} onChange={(e) => handleAreaTematicaChange(e, index, 2)}>
                                                        {listaAreaTematica2.map((area, i) => (
                                                            <option key={i} value={area}>{area}</option>
                                                        ))}
                                                    </Form.Select>
                                                    <Link><Trash3 size={20} color='black' className='mx-3' onClick={() => handleRemoveAreaTematica(index, 2)} /></Link>
                                                </Col>
                                            </div>
                                        ))}
                                    </Form.Group>
