...
function Login() {
  ...
  const handleFacebookAuth = () => {
    window.location.href =
      env.authAccessPoint +
      "/login/facebook?returnUrl=" +
      env.interfaceAccessPoint;
  };

  const handleGoogleAuth = () => {
    window.location.href =
      env.authAccessPoint +
      "/login/google?returnUrl=" +
      env.interfaceAccessPoint;
  };

  const handleSubmit = (event) => {
    axios
      .post(env.authAccessPoint + "/login", {
        username: username,
        password: password,
      })
      .then((response) => {
  ...
  return <>{isSubmitted ? <Navigate to="/" /> : loginForm}</>;
}
